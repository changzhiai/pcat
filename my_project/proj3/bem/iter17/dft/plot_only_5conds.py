...
def plot_scores_vs_chem(df, i, target_Pd_chem_pot, **kwargs):
    ...
    df = df[(df['kappa']==kappa[0]) & (df['d_mu_Pd']==target_Pd_chem_pot)  # change here
            & (df['d_mu_Ti']==d_mu_Ti[0]) & (df['T']==T[1]) # x axis back and forward
            & (df['P_CO']==P_CO[4])]
    df = df.sort_values('U')
    x_col = 'U'
    x = df[x_col]
...
def plot_surf_free_vs_x(dfs, **kwargs):
    ...
    if 'df_raw' in kwargs:
        df_raw = kwargs['df_raw']
    d_mu_Pd = sorted(set(df_raw['d_mu_Pd']))
    Pd_chem_pot = list((d_mu_Pd))
    cands, ids = [], []
    for ind, target_Pd_chem_pot in enumerate(Pd_chem_pot):
        cand, id_set = plot_multi_scores_vs_chem(dfs, target_Pd_chem_pot, ind, **kwargs)
        cands.append(cand)
        ids.append(id_set)
    return cands, ids
...
if __name__ == '__main__':
    ...
    cands, ids = plot_surf_free_vs_x(dfs, **{'plot_all': False})
    plt.show()
