...
def plot_scores_vs_chem_with_Us(dfs, **kwargs):
    """fix U, plot scores vs. pH"""
    Pd_chem_pot = (np.array([0, -0.25, -0.5, -0.75, -1.0, -1.25])+(-2.24900)).tolist()
    cands = []
...
if __name__ == '__main__':
    kappa = np.array([0, 1, 2])
    raw_niches = pd.read_csv('./data/em_tasks_chem_U.csv')
    plt.figure(dpi=300)
    fittest_images = read('./data/fittest_images_chem_U.traj', ':')
    # fittest_images2 = read('fittest_images_1_6.traj', ':')
    # fittest_images += fittest_images2
