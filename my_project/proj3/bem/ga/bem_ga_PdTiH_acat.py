...
from ase.ga.slab_operators_hydride import AdsorbateSubstitute
...
from acat.adsorption_sites import ClusterAdsorptionSites, SlabAdsorptionSites
...
def generate_initial_ga_db(db_name, pop_size):
    if os.path.exists(db_name):
        os.system('cp {} temp.db'.format(db_name))
        os.remove(db_name)
    ...
    images = read('./random50_adss.traj', index=':')
    random.seed(888)
    for atoms in images:
        if 'data' not in atoms.info:
            atoms.info['data'] = {}
...
def relax(atoms, single_point=True):
    ...
    # print(atoms.info['data']['ads_indices'])
    # print(atoms.info['data']['ads_symbols'])
    t2 = time.time()
...
def relax_offspring(n):
    ...
    while True:
        ...
        # _, _ = check_adsorbates(a1)
        # _, _ = check_adsorbates(a2)
    ...
    # print([atom.tag for atom in a3])
    # print(a3.info)
    t2 = time.time()
    print('offspring relaxing:', t2-t1)
    return a3
...
if __name__ == '__main__':
    ...
    if continue_ga == False:
        db = generate_initial_ga_db(db_name, pop_size)
    if copy_to_scratch:
        db, fname = copy_to_scratch(db_name)
    else:
        db = DataConnection(db_name)

    # op_selector = OperationSelector([5., 5, 2., 2, ],
    #                             [RandomSlabPermutation(element_pools=['Pd', 'Ni'], num_muts=5),
    #                             RandomSlabPermutation(element_pools=['H', 'X'], num_muts=5),
    #                             SymmetrySlabPermutation(element_pools=['Pd', 'Ni'], num_muts=5),
    #                             SymmetrySlabPermutation(element_pools=['H', 'X'], num_muts=5),
    #                             # AdsorbateSubstitute
    #                             # AdsorbatePermutation
    #                             # CutSpliceSlabCrossover(element_pools=['Pd', 'Ni'], num_muts=5),
    ...
    #                             # SymmetrySlabPermutation(element_pools=['Pd', 'Ni'], num_muts=5),
    #                             # SymmetrySlabPermutation(element_pools=['H', 'X'], num_muts=5),
    #                             AdsorbateSubstitute(ads_pools=['CO', 'OH', 'H'], num_muts=1)
    #                             # AdsorbatePermutation
    #                             # CutSpliceSlabCrossover(element_pools=['Pd', 'Ni'], num_muts=5),
    #                             # CutSpliceSlabCrossover(element_pools=['H', 'X'], num_muts=5),
    #                             ])
    ...
    for i in range(max_gens):
        ...
        print('Creating and evaluating generation {0}'.format(gens_running))
