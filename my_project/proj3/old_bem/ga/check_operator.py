...
# from comparators_hydride import DataComparator
...
def create_file_logger(name='log.basic_ga'):
...
def single_painn_calculator(args):
...
def ensemble_painn_calculator(args):
...
def relax_single_calc(atoms, single_point=True):
    """Relax one atoms"""
    calc = single_painn_calculator(args)
    ...
    if not single_point:
        ...
        converged = opt.run(fmax=0.1, steps=200)
    Epot = atoms.get_potential_energy()
    atoms.info['key_value_pairs']['potential_energy'] = Epot # for compator
    atoms.info['key_value_pairs']['energy_var'] = ens['energy_var'] # for compator
    atoms.info['data']['nnmat_string'] = get_nnmat_string(atoms, 2, True) # decimal is 2
...
def relax_ensemble_calc(atoms, single_point=False):
    """Relax one atoms ensemble"""
    calc = ensemble_painn_calculator(args)
    mask = [atom.z < 2.0 for atom in atoms]
    atoms.set_constraint(FixAtoms(mask=mask))
    atoms.calc = calc
    converged = True
    ...
    atoms.info['key_value_pairs']['potential_energy'] = Epot # for compator
    atoms.info['key_value_pairs']['energy_var'] = ens['energy_var'] # for compator
    atoms.info['key_value_pairs']['step'] = step # for compator
    atoms.info['key_value_pairs']['converged'] = converged # for compator
    atoms.info['data']['nnmat_string'] = get_nnmat_string(atoms, 2, True) # decimal is 2
    atoms.info['data']['atoms_string'] = ''.join(atoms.get_chemical_symbols())
    std = np.sqrt(ens['energy_var']) # standard variance?
    A_surf = np.linalg.norm(np.cross(atoms.cell[0], atoms.cell[1]))
    # gammas = niches.apply(gamma, axis=1).values
    gammas = []
    for i, r in niches.iterrows():
        gamma = calculate_gamma(atoms, Epot, T=r['T'], U=r['U'], pH=r['pH'], P_H2=r['P_H2'], P_CO2=r['P_CO2'], P_H2O=r['P_H2O'], P_CO=r['P_CO'])
        gammas.append(gamma)
    gammas = np.array(gammas)
    # print(gammas)
    scores = (niches['kappa'].values * std) / A_surf - gammas 
    atoms.info['data']['gammas'] = gammas
...
def relax(atoms, ensemble_calc=True):
    ...
    if distortion:
        ...
        # atoms = atoms_old
        # atoms = relax_ensemble_calc(atoms, single_point=True)
        logger.debug(f"distortion - {distortion} - {reason}")
    return atoms

def relax_init_atoms(atoms, ensemble_calc=True):
    logger.info(f"relaxing atoms {atoms.info['confid']}")
    atoms_old = copy.deepcopy(atoms)
    if ensemble_calc:
        atoms = relax_ensemble_calc(atoms, single_point=False)
...
def relax_offspring(n):
    ...
    while True:
        ...
        try:
            ...
            # write('atoms1.traj', a1)
            # write('atoms2.traj', a2)
            assert False
...
def clean_before_starting():
    db_name_dist='distortion_init.db'
    pop_log = 'pop_multitask.log'
    debug_traj = 'debug.traj'
    remove_list = [db_name_dist, pop_log, debug_traj]
    for item in remove_list:
        if os.path.exists(item):
            os.remove(item)

if __name__ == '__main__':
    images = read('debug.traj', ':')
    a1 = images[0]
    a2 = images[1]
    print(len(a1), len(a2))
    # for atoms in images:
    ...
    #     if_too_close = check_too_close_after_optimized(atoms, cutoff=1.7)
    #     print(if_too_close)
    acsc = AdsorbateCutSpliceCrossover(ads_pools=['CO', 'OH', 'H'], num_muts=1)
    acsc.get_new_individual([a1, a2])
