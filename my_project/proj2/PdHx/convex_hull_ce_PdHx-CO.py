...
def formation_energy_CO_ref_hydr_and_metals(atoms, energy):
    """
    Pure PdH111-CO: -5.22567 eV/atom
    Pure Pd111-CO: -1.58852 eV
    ...
    try:
        ...
        Hs = 0
    form_e = (energy - Hs*(-5.22567) - (64-Hs)*(-1.58852))/(Pds+Hs)
    # print(Pds, Tis, Hs, form_e)
    return form_e
...
def db2xls(db_name):
    ...
    for row in db.select():
        ...
        atoms = row.toatoms()
        ce_e = row.energy
        form_energy = formation_energy_CO_ref_hydr_and_metals(atoms, ce_e)
        form_energies.append(form_energy)
...
def plot_convex_hull_PdHx(db_name, cand=False):
    ...
    for con_H in set(cons_H):
        ...
        df_sub = df_sub[df_sub.form_energies == df_sub.form_energies.min()]
        df_convex = df_convex.append(df_sub, ignore_index=True)
        
    cons_H = df_convex['cons_H']
    form_energies = df_convex['form_energies']
    ids = df_convex['ids']
    ...
    # plt.ylim([0., 0.5])
    plt.title('Convex hull of PdHx-CO')
...
def get_PdHx_candidates(cand_ids, db_name):
    ...
    db_cand_name='candidates_CO_PdHx_r1.db'
    db = connect(db_name)
    if os.path.exists(db_cand_name):
        os.remove(db_cand_name)
    db_cand = connect(db_cand_name)
