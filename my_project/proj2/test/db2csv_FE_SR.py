...
def db2xls(system_name, xls_name, db, sheet_free_energy, sheet_binding_energy, sheet_selectivity):
    """
    ...
    """
    Save all intermediate`s free energy sheet to excel
    """
    tuples = {'Surface': surfaces,
              'G(*HOCO)': G_HOCOs,
              'G(*CO)': G_COs,
    ...
    with pd.ExcelWriter(xls_name, engine='openpyxl', mode='a') as writer:
        df_all_FE.to_excel(writer, sheet_name='All_FE', index=False, float_format='%.3f')
...
def plot_selectivity(xls_name, sheet_selectivity):
    ...
    name_fig_select = '../figures/{}_{}.jpg'.format(system_name, sheet_selectivity)
...
if __name__ == '__main__':
    ...
    db = connect(db_name)
    if True:
        db2xls(system_name, xls_name, db, sheet_free_energy, sheet_binding_energy, sheet_selectivity)
    plot_free_enegy(xls_name, sheet_free_energy,)
    plot_scaling_relations(xls_name, sheet_binding_energy)
    plot_selectivity(xls_name, sheet_selectivity)
