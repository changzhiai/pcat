...
def db2xls(system_name, 
    ...
    custom_dict = {'surface':0, 'HOCO': 1, 'CO': 2} 
    for id in uniqueids:
    ...
    for id in uniqueids:
        ...
        # update:
        Binding_energy = [Eb_HOCO, Eb_CO]
        Free_energy = [G_HOCO, G_CO] # free energy according to reaction equations
        df_stack = pd.concat([HOCO, CO], axis=0)
        df_stack['BE'] = Binding_energy
        df_stack['FE'] = Free_energy
        df_new = df_new.append(df_stack, ignore_index=True)
