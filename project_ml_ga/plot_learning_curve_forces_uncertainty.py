...
def plot_energy_var(df):
    ...
    plt.legend()
    plt.show()
    fig.savefig('learning_curve_energy_uncertainty.png', dpi=300)

def plot_forces_var(df):
    plt.xlabel('Number of step')
    mean = df.mean(axis=1)
    var = df.var(axis="columns")
    ax.plot(df.index, mean, color='red', label='Average')
    ...
    plt.legend()
    plt.show()
    fig.savefig('learning_curve_forces_uncertainty.png', dpi=300, bbox_inches='tight')
